...
def naive_softmax_loss_and_gradient(
    ...
    ### YOUR CODE HERE
    num_out_vectors = outside_vectors.shape[0]
    scores = np.matmul(center_word_vec, outside_vectors.T)
    y_hat = softmax(scores)
    prob_o = y_hat[0, outside_word_idx]
    loss = -np.log(prob_o)

    y = np.zeros(y_hat.shape)
    y[0, outside_word_idx] = 1
    temp_v = y_hat - y
    grad_center_vec = np.matmul(temp_v, outside_vectors)

    center_word_vec_expand = np.tile(center_word_vec, (num_out_vectors, 1))
    grad_outside_vecs = (np.multiply(center_word_vec_expand.T, temp_v)).T
    ### END YOUR CODE
...
def neg_sampling_loss_and_gradient(
    ...
    ### YOUR CODE HERE
    num_out_vectors = outside_vectors.shape[0]
    activations = np.zeros(K + 1)
    for i in range(K + 1):
        out_vec = outside_vectors[indices[i], :]
        activations[i] = sigmoid(np.dot(center_word_vec, out_vec))
    loss = -np.log(activations[0])
    neg_sample_activations = activations[1:]
    neg_sample_activations_minus = np.log(1 - neg_sample_activations)
    loss -= np.sum(neg_sample_activations_minus)

    u_vectors = outside_vectors[neg_sample_word_indices, :]
    grad_center_vec = (activations[0] - 1) * outside_vectors[outside_word_idx]
    temp_sum = np.matmul(neg_sample_activations, u_vectors)
    grad_center_vec += temp_sum

    grad_outside_vecs = np.zeros(outside_vectors.shape)
    grad_outside_vecs[outside_word_idx: outside_word_idx + 1] += -(1 - activations[0]) * center_word_vec
    for i, negative_sample_index in enumerate(neg_sample_word_indices):
        grad_outside_vecs[negative_sample_index: negative_sample_index + 1] += activations[i + 1] * center_word_vec
...
def skipgram(current_center_word, outside_words, word2ind,
    ...
    center_word_vec = center_word_vectors[center_word_index: center_word_index + 1]

    for outside_word in outside_words:
        outside_word_idx = word2ind[outside_word]
        curr_loss, curr_center_vec_grad, curr_grad_outside_vectors = word2vec_loss_and_gradient(center_word_vec,
                                                                                                outside_word_idx,
                                                                                                outside_vectors,
                                                                                                dataset)
        loss += curr_loss
        grad_center_vecs[center_word_index: center_word_index + 1] += curr_center_vec_grad
        grad_outside_vectors += curr_grad_outside_vectors
    ### END YOUR CODE
